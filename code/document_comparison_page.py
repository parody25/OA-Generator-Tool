...
def generate_templated_pdf(memo_content, template_path="constant/template.json"):
    ...
    elements = []
    # Add header
    # Iterate over each section and its content
    for section in template["template"]["sections"]:
        content_key = section.get("content_key", None)

        if content_key:
            content = memo_content.get(content_key, None)
            if content is None:
           ...
           # elements.append(Paragraph(content, style_body))
            if section["title"] == "Highlights of the New Service":
                image_path = "assets/representation.webp"
                image = Image(image_path, width=200, height=100)
                elements.append(image)

        # Check for subsections and process them
        if "subsections" in section:
            for subsection in section["subsections"]:
                sub_content_key = subsection.get("content_key", None)

                if sub_content_key:
                    sub_content = memo_content.get(sub_content_key, None)
                    if sub_content is None:
                        sub_content = "Content not available."
                    if section["title"] != "Responsibilities" or sub_content != "Content not available.":
                        elements.append(Paragraph(subsection["title"], style_section))
                        points = process_points(sub_content)
                        elements.extend(points) 
                        #elements.append(Paragraph(sub_content, style_body))
                else:
                    # If no content_key is provided, mark as unavailable
                    elements.append(Paragraph(subsection["title"], style_section))
                    elements.append(Paragraph("Content not available.", style_body))
...
def generate_arabic_pdf(memo_content, template_path="constant/template.json"):
    # Load the template JSON
    pdfmetrics.registerFont(TTFont('ArabicFont', "assets/ArabicFont/NotoKufiArabic-Regular.ttf"))
    with open(template_path, "r") as f:
        template = json.load(f)
...
def generate_gpt4_memo(doc1_text, doc2_text, responsibilties_selected, stakeholders_context):
    ...
    responsibilities_string = ""   #To add in the **Responsibilties** line in prompt
    keys_string = ""   #To add in the JSON format keys

    #Branch, Service Manager, Account Opening Team, Compliance Team & Online Banking Team
    #"branch_responsibilities", "service_manager_responsibilities", "account_opening_team_responsibilities", "compliance_team_responsibilities", "online_banking_team_responsibilities"
    ...
    **Document Differences:**
    ...
    Based on these differences, provide detailed, actionable outputs for each of the following sections. Your response must be specific, step-by-step, and tailored to the context of operational advice. For every section (except the summary), provide a numbered list of detailed steps.

    **Required Sections:**
    - **Summary of the changes:** Provide a well-written paragraph summarizing the key changes between the documents. Explain what has changed, why these changes matter, and any overall implications.
    - **Highlights of the new service:** List the specific features or improvements introduced by the changes. Provide a detailed, numbered, point-by-point list.
    - **Potential impact or concerns:** Identify specific operational, compliance, or customer-related risks and benefits. For each, provide examples or scenarios in a numbered list.
    - **Actionable recommendations:** Provide clear actions that teams should take to address the changes, mitigate risks, or maximize benefits. List these as a step-by-step, numbered list.
    - **Responsibilities** ({responsibilities_string}): For each team, specify concrete tasks or duties that are directly tied to their roles and the changes. Provide a detailed, numbered list of at least 5-7 specific actions per team.
    ...
    **Response Format:**  
    ...
    - The "summary_of_changes" value should be a well-crafted paragraph.
    - The values for all other keys must be a single string that contains a numbered list, where each new step is on a new line and prefixed with a number (e.g., "1.", "2.", "3.", etc.).

    Ensure that every section is as detailed as possible and that your instructions are clear and actionable.
    """
    print("Prompt", prompt)
    try:
...
def document_comparison_page():
    st.header("Enhanced Document Comparison and Operational Advice Memo 📝")
